...
Thanks for considering a contribution to `assets-web`.
...
A local Redis is required (used for caching, sessions, and rate limiting) - `redis-server` on
`localhost:6379` with no auth is sufficient for tests.
